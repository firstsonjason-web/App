...
exports.createStripeCustomer = functions.auth.user().onCreate(async (user) => {
   try {
     ...
     await reportError(error, { user: user.uid });
     // Optionally, set an error in Firestore or handle differently
     throw error; // Re-throw to prevent user creation if critical
   }
 });
   ...
   .onUpdate(async (change, context) => {
     if (change.after.data().status === 'requires_confirmation') {
       try {
         ...
         functions.logger.error('Error confirming Stripe payment:', error);
...
exports.cleanupUser = functions.auth.user().onDelete(async (user) => {
   try {
     ...
     functions.logger.error('Error cleaning up user:', error);
     await reportError(error, { user: user.uid });
     throw error; // Re-throw to ensure cleanup is retried or logged
   }
 });
...
exports.createPaymentIntent = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    let customerDoc = await admin.firestore().collection('stripe_customers').doc(userId).get();
    let customerId;
